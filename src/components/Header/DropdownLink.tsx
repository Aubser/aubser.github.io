...
export default function DropdownLink({ item, index, activeIndex, setActiveIndex }: DropdownLinkProps) {
   ...
   return (
      <div className="relative flex-1 h-full w-full flex" ref={hoverRef} onMouseEnter={() => setActiveIndex(index)}
         onMouseLeave={() => setActiveIndex(null)}>
         {/* Button */}
         <Link
            href={item.href}
            className={`
               px-4 py-2 inline-flex items-center justify-center whitespace-nowrap
